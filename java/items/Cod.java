...
public class Cod implements Items {
	public static ItemStack createCustomItem() {
		ItemStack cod = new ItemStack(Material.COD);
		ItemMeta data = cod.getItemMeta();
		data.setDisplayName(ChatColor.WHITE + "Cod");
	...
	public void onRightClick(Player p) {
		...
		if(health + 2 > 20) {
			p.setHealth(20.0);
			p.sendMessage(ChatColor.GREEN + "" + ChatColor.BOLD + "You ate a Cod and are now at full health!");
		} else {
			p.setHealth(health + 2);
			p.sendMessage(ChatColor.GREEN + "" + ChatColor.BOLD + "You ate a Cod and healed for 2 HP.");
		}
	}

	@Override
	public void onLeftClick(Player p) {
		PlayerListener.shootBeam(p, Color.MAROON, 64, 1, 4);
	}
}
